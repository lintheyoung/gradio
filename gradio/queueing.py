...
class Queue:
    ...
    async def call_prediction(self, events: list[Event], batch: bool):
        ...
        try:
            my_body_data = body.data[1]

            if my_body_data != None:
                # print("here is = ", my_body_data)
                if my_body_data[0] == '{':
                    my_body_data_dict = eval(my_body_data)
                    # print(my_body_data_dict)

                    if my_body_data_dict['key'] == 'Positive':
                        body.data[1] = 'Positive'
                        print('Positive')
                    if my_body_data_dict['key'] == 'Negative':
                        body.data[1] = 'Negative'
                        print('Negative')

                    body.event_data = {"index": my_body_data_dict['index'],"value": None}
        except:
            print("not target")
                
            
        
        # print("body.event_data = ", body.event_data)
